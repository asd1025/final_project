...
public class CategoryController {
	...
	public ResponseEntity<JSONResult> regCategory(@RequestBody CategoryVo categoryVo) {
		boolean isAddCategory = categoryService.regCategory(categoryVo);
		if(!isAddCategory){
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(JSONResult.fail("카테고리 등록 실패"));
		}
		else return ResponseEntity.status(HttpStatus.OK).body(JSONResult.success(categoryVo));
	}
